...
const spell = (pattern, correct) => ({
  pattern,
  check(match) {
    let expected = typeof correct === 'function' ? correct(match) : correct
    let message;
    if (Array.isArray(expected)) {
      if (!expected.includes(match[0])) {
        expected = expected.map(x => `“${x}”`).join(' / ')
        message = `“${match[0]}” → ${expected}`
      }
    } else {
      if (match[0] !== expected) {
        message = `“${match[0]}” → “${expected}”`
...
const rules = [
  ...
  },
  {
    ...
    check(match) {
      if (match[1] !== 'ๆ') {
        return {message: 'Add a space after ๆ', expected: "ๆ "}
      }
      return {}
    },
  },
  ...
  spell(/เชอปราง(ค์)?/g, 'เฌอปราง'), //Of course

  // ราชบัณฑิตยสถาน. อ่านอย่างไรและเขียนอย่างไร ฉบับราชบัณฑิตยสถาน (แก้ไขเพิ่มเติม) .-- พิมพ์ครั้งที่ 22.-- กรุงเทพฯ : ราชบัณฑิตยสถาน, 2557
  spell(/กงศุล/g, 'กงสุล'),
  spell(/กฏหมาย/g, 'กฎหมาย'),
  spell(/กลบปาก/g, 'กบปาก'),
  spell(/กบ(?:ฎ|ถ)/g, 'กบฏ'),
  spell(/กัน(ไกร|เชียง|โชก|ไตร)/g, match => `กรร${match[1]}`),
  spell(/กรรมบท/g, 'กรรมบถ'),
  spell(/กรรมพันธ์/g, 'กรรมพันธุ์'),
  spell(/กรรมสิทธิ(?!์)/g, 'กรรมสิทธิ์'),
  spell(/ตรวจน้ำ/g, 'กรวดน้ำ'),
  spell(/กัดกล่อน/g, 'กัดกร่อน'),
  spell(/กร?ะจิ[่้๊๋]?ดริ๊?ด/g, 'กระจิริด'),
  spell(/กร?ะเฌอ/g, 'กระเชอ'),
  spell(/กร?ะตือรือ(ร|ล)้น/g, 'กระตือรือร้น'),
  spell(/กะเทือน/g, 'กระเทือน'),
  spell(/ขบวนการยุติธรรม/g, 'กระบวนการยุติธรรม'),
  spell(/กร?ะเบียดกร?ะเสีย(ล|น)/g, 'กระเบียดกระเสียร'),
  spell(/(กะเบียดกร?ะ|กร?ะเบียดกะ)เสียร/g, 'กระเบียดกระเสียร'),
  spell(/กะปร?ี้กร?ะเป(ร|ล)?่า/g, 'กระปรี้กระเปร่า'),
  spell(/กระสันต์/g, 'กระสัน'),
  spell(/กระแสร์น้ำ/g, 'กระแส'),
  spell(/กะหืดกะหอบ/g, 'กระหืดหระหอบ'),  
]
    ...
    for (const r of rules) {
      ...
      for (const pattern of [].concat(r.pattern)) {
        ...
        for (
          ...
          if (!Array.isArray(file.expected)) file.expected = []
          const point = {
          ...
          if (r.check) {
            const test = r.check(match)
            if (test.message) {
              file.message(test.message, point)
              file.expected.push({point, expected: test.expected})
